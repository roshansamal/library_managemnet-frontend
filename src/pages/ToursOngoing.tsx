...
export default function ToursOngoing() {
  ...
  const handleFetch = async () => {
    try {
      setIsLoading(true);
      const params = new URLSearchParams({ ticket_status: "Started" });
      const token = localStorage.getItem('authToken');
      const apiUrl = import.meta.env.VITE_API_URL ?? 'https://localhost:8000';
      const res = await fetch(`${apiUrl}/api/touradmin/tours-by-status?${params}`, {
      //const res = await fetch('/api/tourbill/tourongoing', {
          method: 'GET',
          credentials: 'include',
          headers: {
  ...
  return (
    ...
    <Breadcrumb fontWeight="normal" fontSize={"sm"} mb={1} ml={1} color={"red"}>
      ...
      <BreadcrumbItem isCurrentPage>
        <BreadcrumbLink>Ongoing Tours</BreadcrumbLink>
      </BreadcrumbItem>
    </Breadcrumb>
