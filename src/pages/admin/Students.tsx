...
import { FiDownload, FiSearch,FiChevronLeft, FiChevronRight, FiEdit } from "react-icons/fi";
import { FaCross, FaDeleteLeft, FaRecycle, FaTrash } from 'react-icons/fa6';
...
import AddStudentSlider from '../../components/admin/AddStudentSlider';
const columnHelper = createColumnHelper<StudentMasterType>();

export default function Students() {
  const [searchText, setSearchText] = useState("");
  const [selectedRow, setSelectedRow] = useState<StudentMasterType | null>(null);
  ...
  const [isStudentAddFormOpen, setIsStudentAddFormOpen] = useState(false);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [rowToDelete, setRowToDelete] = useState<StudentMasterType | null>(null);
  const toast = useToast();
  const token = localStorage.getItem('authToken');
  const [isExporting, setIsExporting] = useState(false);
  const [pagination, setPagination] = useState({ pageIndex: 0, pageSize: 10 });
  const [data, setData] = useState<StudentMasterType[]>([]);
  ...
  const closeStudentAddForm = () => {
    setSelectedRow(null);
    setIsStudentAddFormOpen(false);
  };
...
const handleFetch = useCallback(
  async (opts?: { search?: string }) => {
    ...
    const res = await fetch(
      `${apiUrl}/api/students/list?${params.toString()}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
...
// const canNext = table.getCanNextPage();
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const apiUrl = import.meta.env.VITE_API_URL ?? 'https://localhost:8000';
      const res = await fetch(`${apiUrl}/api/students/delete`, {
        method: 'DELETE',
        headers: {
      ...
      toast({
        title: 'Deleted',
        description: `Employee ${rowToDelete.rollno} deleted successfully.`,
        status: 'success',
        duration: 3000,
        isClosable: true,
      });
  ...
  return (
    ...
    <Flex align="center" as="nav" aria-label="Breadcrumb" w="100%" py={1}>
      ...
      <HStack spacing={1} display={{ base: "none", md: "flex" }}>
        ...
        <Text fontWeight="normal">Students</Text>
      </HStack>
    ...
    <EmpEditSlider
      ...
      /> */}
    <AddStudentSlider
            isOpen={isStudentAddFormOpen}
            onClose={closeStudentAddForm}
            initialData={selectedRow}
     />
    <ConfirmDeleteDialog
      isOpen={isDeleteOpen}
